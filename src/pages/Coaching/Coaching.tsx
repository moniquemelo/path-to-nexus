      ...
      try {
        ...
        if (userIds && userIds.length > 0) {
          ...
          const { data: reviews, error: reviewsError } = await supabase
            ...
            .in('coach_id', data.map((coaching: Coaching) => coaching.id));
          ...
          const coachingsWithUsersAndRatings = (data as Coaching[]).map((coaching) => {
            const user = users?.find((u) => u.id === coaching.user_id);
            const coachingReviews = reviews?.filter((review: { coach_id: string }) => review.coach_id === coaching.id) || [];
          ...
          <div className={styles.cardUsernameAndRating}>
            ...
            <div className={styles.ratingContainer}>
              ...
              <span className={styles.rating}>
                {coaching.averageRating !== undefined
                  ? coaching.averageRating.toFixed(1)
                  : 'Sem avaliações'}
                ({coaching.ratingCount} {coaching.ratingCount === 1 ? 'nota' : 'notas'})
              </span>
            </div>
          </div>
